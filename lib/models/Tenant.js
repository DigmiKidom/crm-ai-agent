...
import { MAX_CATALOG_ITEMS as CATALOG_LIMIT, MAX_GALLERY_PHOTOS } from "@/lib/plan";
import {
...
// The schema ceiling, NOT the per-tenant allowance: what a given tenant may
// save depends on their plan (six on Free, fifteen on Pro — see
// galleryLimit() in lib/plan.js, enforced by the PATCH route). The two are
// kept apart on purpose so that a lapsed subscription doesn't turn a saved
// page into a document its own schema rejects.
// Re-exported under the names the rest of the app already imports, rather
// than via `export ... from`, which would leave no local binding for the
// schema validators below to read.
export const MAX_GALLERY = MAX_GALLERY_PHOTOS;
export const MAX_CATALOG_ITEMS = CATALOG_LIMIT;
...
const TenantSchema = new mongoose.Schema(
  {
    ...
    landingPage: {
      ...
      // ── Item list (Pro) ───────────────────────────────────────────────────
      //
      // A menu, price list or product range, rendered as cards between the
      // gallery and the contact form. See lib/catalog.js for what a row may
      // contain and why `price` is free text.
      //
      // Explicitly NOT commerce: no cart, no checkout, no stock, no payment.
      // The section's only action is a link down to the lead form, which is
      // the whole point — the business wants the conversation, and this
      // product has no way to take money on their behalf.
      //
      // Empty means the section doesn't render at all, which is the correct
      // state for every tenant who existed before this shipped.
      catalogHeading: { type: String, default: "" },
      // One line under the heading, for the caveats every price list needs
      // ("prices include VAT", "minimum order 10"). Optional.
      catalogNote: { type: String, default: "", maxlength: MAX_CATALOG_NOTE },
      catalog: {
        type: [
          {
            title: { type: String, default: "", maxlength: MAX_CATALOG_TITLE },
            description: { type: String, default: "", maxlength: MAX_CATALOG_DESCRIPTION },
            // A label, never a number — see lib/catalog.js.
            price: { type: String, default: "", maxlength: MAX_CATALOG_PRICE },
            // Points at a Media document, same as every other image on this
            // page; the bytes never live on the tenant doc.
            mediaId: { type: mongoose.Schema.Types.ObjectId, ref: "Media", default: null },
          },
        ],
        default: [],
        validate: {
          validator: (v) => v.length <= CATALOG_LIMIT,
          message: `At most ${CATALOG_LIMIT} items.`,
        },
      },
    },
    ...
    // Daily quota for the AI website design agent (see lib/aiUsage.js).
    //
    // On the tenant, not the user: a business has one landing page, so three
    // regenerations a day is a budget for that page. Counting per user would
    // let a three-person team spend nine, which is the same page rewritten
    // nine times and nine times the model cost for no extra value.
    //
    // `lastResetDate` is midnight of the UTC day the count belongs to, so
    // "is this counter still current?" is a date comparison rather than a
    // stored expiry that has to be swept. A tenant created before this field
    // existed reads as an untouched day — see readAiDesignUsage().
    aiDesignGenerations: {
