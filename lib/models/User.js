...
const UserSchema = new mongoose.Schema(
  {
    ...
    // Which optional tools this person has switched on — ids from
    // lib/plugins.js, empty for everyone until they visit Settings → Tools.
    //
    // On the user rather than the tenant, even though the tools' *data* is
    // shared across the team: which rows are in your sidebar is a preference
    // about your own screen, and one person tidying their nav must not remove
    // a colleague's. The data those rows lead to stays tenant-scoped, so
    // turning a tool off hides it and never deletes anything.
    //
    // Read through normalizeEnabledPlugins() rather than directly: this array
    // outlives the registry, so it can hold the id of a tool that no longer
    // exists.
