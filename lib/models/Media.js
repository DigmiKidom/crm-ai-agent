...
const MediaSchema = new mongoose.Schema(
  {
    ...
    // "logo" | "background" | "gallery" | "catalog" | "avatar" — lets us clean
    // up or quota by purpose later. Avatars are per-USER but still carry a
    // tenantId, so the same tenant-scoped cleanup and quota logic covers them.
    //
    // "catalog" is separate from "gallery" even though both are landing-page
    // photos: they are cropped differently, and a future cleanup that drops a
    // tenant's gallery must not silently take their price list with it.
    kind: {
      type: String,
      enum: ["logo", "background", "gallery", "catalog", "avatar"],
      required: true,
    },
