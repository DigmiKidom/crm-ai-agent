    ...
    parametersJsonSchema: {
      ...
      properties: {
        ...
        faqHeading: {
          type: "string",
          description:
            "Heading above the FAQ accordion, e.g. 'Frequently asked questions'. Same language as the rest of the copy.",
        },
        faq: {
          type: "array",
          description:
            "4 to 5 questions a real prospective customer of THIS business would ask before making contact — pricing, timelines, coverage area, what happens after they enquire, guarantees. Answer each one the way this business would. Same language as the rest of the copy.",
          items: {
            type: "object",
            properties: {
              question: { type: "string", description: "The question, as a customer would phrase it." },
              answer: {
                type: "string",
                description:
                  "A direct, reassuring answer in 1-3 sentences, 600 characters maximum. Never invent a specific price, phone number, or address — speak in terms the business can stand behind without checking.",
              },
            },
...
/**
 * FAQ-only generation, for the "Write these for me" button in the landing
 * page editor.
 *
 * Separate from generateSiteConfig() rather than a re-run of it: the tenant
 * has already written (or edited) their page by this point, so this is fed
 * their actual live copy and asked to answer the questions that copy leaves
 * open — regenerating the whole site config would overwrite that copy to
 * produce an FAQ for it, which is backwards.
 */
export async function generateFaq({
  ...
  const declaration = {
    name: "generate_faq",
    description:
      "Write the questions a prospective customer of this specific business would ask before contacting them, with the answers that business would give.",
    parametersJsonSchema: {
      type: "object",
      properties: {
        faq: {
          ...
          items: {
            ...
            properties: {
              ...
              answer: {
                type: "string",
                description: "1-3 sentences, 600 characters maximum.",
              },
            },
            required: ["question", "answer"],
          },
          minItems: Math.min(3, count),
          maxItems: count,
        },
      },
      required: ["faq"],
  ...
  const response = await ai.models.generateContent({
    model: MODEL,
    contents: `Write ${count} FAQ entries for the landing page of this business.

Company name: ${companyName}
Industry: ${industry || "not specified"}

Their page currently says:
Headline: ${headline || "(none)"}
Subheadline: ${subheadline || "(none)"}
${featureLines ? `What they highlight:\n${featureLines}` : ""}

LANGUAGE: Write every question and answer in ${languageName || "the same language the copy above is written in"}. The FAQ sits directly above the contact form on that same page — a page in one language with an FAQ in another is broken.

Answer the questions a real customer actually hesitates over before making contact: what it costs or how pricing works, how long it takes, whether they cover the customer's area or need, what happens after someone sends the form, and what guarantees exist. Prefer the concerns specific to this industry over generic ones.

Never invent a concrete price, phone number, address, or statistic — this business has to be able to stand behind every answer without correcting it. Where a real number would be needed, describe how it works instead ("pricing depends on…", "we'll confirm a quote after a short call"). Keep each answer to 1-3 sentences.`,
    config: {
      toolConfig: {
...
Everything you produce is read by that business's own visitors: the headline, the subheadline, the CTA, the feature cards, the FAQ, the pipeline stage names, the section headings, AND the lead form's field labels and status messages. All of it must be in one consistent language. A page whose copy is in one language but whose form labels are in another is broken — do not do that. Then report which language you used in languageCode/languageName.
