...
export const RANGES = {
  week: { key: "week", label: "Week", caption: "Last 7 days", bucket: "day", size: 7 },
  month: { key: "month", label: "Month", caption: "Last 30 days", bucket: "day", size: 30 },
  year: { key: "year", label: "Year", caption: "Last 12 months", bucket: "month", size: 12 },
  "3y": { key: "3y", label: "3 years", caption: "Last 36 months", bucket: "month", size: 36 },
};
...
function bucketLabel(date, bucket, size) {
  const d = new Date(date);
  if (bucket === "month") {
  ...
  if (def.bucket === "day") {
    ...
    for (let i = 0; i < def.size; i++) {
      ...
      buckets.push({
        ...
        label: bucketLabel(bStart, "day", def.size),
        start: bStart,
    ...
    for (let i = 0; i < def.size; i++) {
      ...
      buckets.push({
        ...
        label: bucketLabel(bStart, "month", def.size),
        start: bStart,
 ...
 */
export async function getAnalytics({ tenantId, range: rangeKey }) {
  ...
  const previous = leads.filter((l) => {
    ...
    return t >= range.prevStart && t < range.start;
  });
  ...
  for (const lead of previous) {
    ...
    const offset =
      range.bucket === "day"
        ? Math.floor((startOfDay(t) - range.prevStart) / DAY_MS)
        : (t.getFullYear() - range.prevStart.getFullYear()) * 12 +
          (t.getMonth() - range.prevStart.getMonth());
  ...
  const RESPONSE_BANDS = [
    { label: "Under 1 hour", max: HOUR_MS },
    { label: "1–4 hours", max: 4 * HOUR_MS },
    { label: "4–24 hours", max: DAY_MS },
    { label: "1–3 days", max: 3 * DAY_MS },
    { label: "Over 3 days", max: Infinity },
  ];
  const responseBreakdown = RESPONSE_BANDS.map((band, i) => {
    const min = i === 0 ? 0 : RESPONSE_BANDS[i - 1].max;
    const count = responseTimes.filter((t) => t >= min && t < band.max).length;
  ...
  const formCompleteness = [
    { label: "Name", value: current.length, share: 100, required: true },
    { label: "Email", value: current.length, share: 100, required: true },
    { label: "Phone", value: withPhone, share: percent(withPhone, current.length) },
    { label: "Message", value: withMessage, share: percent(withMessage, current.length) },
  ];
  ...
  for (const lead of current) {
    const t = new Date(lead.createdAt);
    heatmap[t.getDay()][t.getHours()] += 1;
    byWeekday[t.getDay()] += 1;
    byHour[t.getHours()] += 1;
  }
  ...
  const kpis = [
    {
      ...
      hint: `${previous.length} in the previous period`,
    ...
    },
    {
      ...
      hint: decided
        ...
        : "No leads have reached a final stage yet",
      spark: winRateOverTime.map((p) => p.value ?? 0),
    },
    {
      key: "response",
      label: "Avg. first response",
      value: humanDuration(avgResponse),
      ...
      hint:
        responseTimes.length > 0
          ? `Median ${humanDuration(median(responseTimes))} · ${responded.length} of ${current.length} opened`
          : "No leads opened yet in this period",
    },
    {
      key: "backlog",
      ...
      hint: oldestUnanswered
        ? `Oldest has been waiting ${humanDuration(oldestUnanswered)}`
        : "Everything has been opened",
    },
    {
      key: "completeness",
      label: "Avg. fields per submission",
      value: current.length ? `${Math.round((totalFieldsFilled / current.length) * 100) / 100} / 4` : "—",
      hint: `${percent(fullyCompleted, current.length)}% filled every optional field`,
    },
    {
      key: "contacts",
      label: "New contacts",
      ...
      hint: `${contacts.length} contacts in total`,
    },
  ];
