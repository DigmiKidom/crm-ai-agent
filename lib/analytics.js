...
// Billing gate: analytics history depth, per the roadmap ("the natural first
// gate is analytics history depth and the advanced panels"). Free tenants
// see week/month; year/3y need Tenant.plan === "pro". Co-located with RANGES
// itself so RangePicker (the hint) and the analytics page (the actual
// enforcement) can't drift apart on which ranges are gated.
export const PRO_ONLY_RANGES = ["year", "3y"];
...
// Re-exported so existing importers (test/analytics.test.mjs) don't need a
// second import — the classifier itself now lives in lib/stageClassifier.js,
// which see for why.
export { classifyStages };
 ...
 */
export async function getAnalytics({ tenantId, range: rangeKey, t, locale = "en", currency = "USD" }) {
  const money = (amount) => formatMoney(amount, currency, locale);
  const range = resolveRange(rangeKey, locale);

  const [leads, openBacklog, pipeline, contacts, totalLeadsAllTime, visitRollups] = await Promise.all([
    // Reaches back to prevStart so the current and previous windows come from
    // a single query.
    Lead.find({ tenantId, createdAt: { $gte: range.prevStart } })
      .select("createdAt stage dealStatus phone message source read readAt updatedAt dealValue")
      .lean(),
    ...
    // Daily rollups (see lib/tracking.js + app/api/track/route.js) — reaches
    // back to prevStart same as leads, so "vs. previous period" works for
    // visits too. A tenant who enabled this feature after prevStart simply
    // has no rows before then; that reads as 0 visits, not an error.
    VisitRollup.find({ tenantId, date: { $gte: bucketKey(range.prevStart, "day") } })
      .select("date visits ctaClicks")
      .lean(),
  ]);
  ...
  // dealStatus is server-derived from the same classification (see
  // app/api/leads/[id]/route.js), so a lead counts as won/lost if EITHER
  // signal says so — a lead whose stage was renamed out from under it after
  // being marked won still counts, rather than silently dropping out of
  // revenue because the regex no longer matches its current stage text.
  const isWon = (l) => wonStages.has(l.stage) || l.dealStatus === "won";
  const isLost = (l) => lostStages.has(l.stage) || l.dealStatus === "lost";
  ...
  const wonCount = current.filter(isWon).length;
  const lostCount = current.filter(isLost).length;
  const openCount = current.length - wonCount - lostCount;
  const decided = wonCount + lostCount;

  const prevWon = previous.filter(isWon).length;
  const prevLost = previous.filter(isLost).length;
  ...
  const winRateOverTime = range.buckets.map((b) => {
    ...
    const w = inBucket.filter(isWon).length;
    const d = w + inBucket.filter(isLost).length;
    return { label: b.label, value: d ? percent(w, d) : null, meta: `${w}/${d} decided` };
  });

  /* --- 2b. Revenue ---------------------------------------------------------
     Same methodology (and same honest limitation) as win rate above: a lead
     counts here if it's CURRENTLY in a won stage and was CREATED in this
     range — not when it was actually won, since that would need per-lead
     stage-change timestamps this query doesn't fetch. A deal closed long
     after being captured shows up in the range it arrived in, not the range
     it closed in. dealValue defaults to 0 ("not valued yet"), so an unvalued
     won lead contributes to the win-rate count above but not to revenue.
     (Lead.wonAt now records the actual close date for anything marked won
     through the app since this shipped — visible on the lead's own timeline —
     but every chart on this page buckets by capture date, so revenue stays
     consistent with the rest of the page rather than switching bases.) */

  const wonLeadsInRange = current.filter(isWon);
  const prevWonLeadsInRange = previous.filter(isWon);

  const revenueInRange = wonLeadsInRange.reduce((sum, l) => sum + (l.dealValue || 0), 0);
  const revenuePrevRange = prevWonLeadsInRange.reduce((sum, l) => sum + (l.dealValue || 0), 0);

  const valuedWonCount = wonLeadsInRange.filter((l) => l.dealValue > 0).length;
  const avgDealSize = valuedWonCount ? Math.round(revenueInRange / valuedWonCount) : null;

  const revenueCounts = new Map(range.buckets.map((b) => [b.key, 0]));
  for (const lead of wonLeadsInRange) {
    const key = bucketKey(lead.createdAt, range.bucket);
    if (revenueCounts.has(key)) revenueCounts.set(key, revenueCounts.get(key) + (lead.dealValue || 0));
  }
  const revenueOverTime = range.buckets.map((b) => ({ label: b.label, value: revenueCounts.get(b.key) ?? 0 }));
  ...
  /* --- 10. Visitor traffic & conversion ----------------------------------
     Rollups are already daily-bucketed (see lib/tracking.js), so "current
     window" is just a string comparison against each row's own date key —
     no per-lead work needed the way leads/contacts required. A tenant with
     zero rollup rows (feature not live yet, or a period before it shipped)
     reads as 0 visits, which is why conversion rate below is null rather
     than a misleading 0% or divide-by-zero. */

  const startKey = bucketKey(range.start, "day");
  const prevStartKey = bucketKey(range.prevStart, "day");

  const visitsInRange = visitRollups
    .filter((r) => r.date >= startKey)
    .reduce((sum, r) => sum + r.visits, 0);
  const visitsPrevRange = visitRollups
    .filter((r) => r.date >= prevStartKey && r.date < startKey)
    .reduce((sum, r) => sum + r.visits, 0);

  const hasVisitData = visitRollups.length > 0;
  const conversionRate = visitsInRange > 0 ? percent(current.length, visitsInRange) : null;
  const prevConversionRate = visitsPrevRange > 0 ? percent(previous.length, visitsPrevRange) : null;

  // Click-through rate on the hero CTA button — a visitor who never scrolled
  // to it, or filled the form some other way, isn't a "no". Same dedup
  // methodology as visits (see lib/tracking.js): one click counted per
  // visitor per day, so re-clicking doesn't inflate it.
  const ctaClicksInRange = visitRollups
    .filter((r) => r.date >= startKey)
    .reduce((sum, r) => sum + (r.ctaClicks || 0), 0);
  const ctaClicksPrevRange = visitRollups
    .filter((r) => r.date >= prevStartKey && r.date < startKey)
    .reduce((sum, r) => sum + (r.ctaClicks || 0), 0);
  const ctr = visitsInRange > 0 ? percent(ctaClicksInRange, visitsInRange) : null;
  const prevCtr = visitsPrevRange > 0 ? percent(ctaClicksPrevRange, visitsPrevRange) : null;

  const visitCounts = new Map(range.buckets.map((b) => [b.key, 0]));
  for (const r of visitRollups) {
    // r.date is always a daily "YYYY-MM-DD" key; re-bucket it to the range's
    // own granularity (day or month) the same way a lead's raw timestamp is,
    // via the same bucketKey() — a year/3y view buckets by month, so several
    // daily rollup rows fold into one point here.
    const [y, m, d] = r.date.split("-").map(Number);
    const key = bucketKey(new Date(y, m - 1, d), range.bucket);
    if (visitCounts.has(key)) visitCounts.set(key, visitCounts.get(key) + r.visits);
  }
  const visitsOverTime = range.buckets.map((b) => ({
    label: b.label,
    value: visitCounts.get(b.key) ?? 0,
  }));
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: valuedWonCount ? money(revenueInRange) : "—",
      delta: valuedWonCount && revenuePrevRange ? delta(revenueInRange, revenuePrevRange) : null,
      hint: valuedWonCount
        ? t("analytics.kpi.fromDeals", { count: valuedWonCount })
        : t("analytics.kpi.noValuedDeals"),
      spark: revenueOverTime.map((p) => p.value),
    },
    {
      key: "avgDealSize",
      label: t("analytics.kpi.avgDealSize"),
      value: avgDealSize != null ? money(avgDealSize) : "—",
      hint: t("analytics.kpi.avgDealSizeHint"),
    },
    {
      key: "response",
    ...
    {
      key: "visitors",
      label: t("analytics.kpi.visitors"),
      value: hasVisitData ? visitsInRange : "—",
      delta: hasVisitData ? delta(visitsInRange, visitsPrevRange) : null,
      hint: hasVisitData
        ? t("analytics.kpi.prevPeriod", { count: visitsPrevRange })
        : t("analytics.kpi.noVisitData"),
      spark: hasVisitData ? visitsOverTime.map((p) => p.value) : undefined,
    },
    {
      key: "conversion",
      label: t("analytics.kpi.conversion"),
      value: conversionRate != null ? `${conversionRate}%` : "—",
      delta: conversionRate != null && prevConversionRate != null ? delta(conversionRate, prevConversionRate) : null,
      hint:
        conversionRate != null
          ? t("analytics.kpi.leadsOfVisitors", { leads: current.length, visits: visitsInRange })
          : t("analytics.kpi.noVisitData"),
    },
    {
      key: "ctr",
      ...
      value: ctr != null ? `${ctr}%` : "—",
      delta: ctr != null && prevCtr != null ? delta(ctr, prevCtr) : null,
      hint:
        ctr != null
          ? t("analytics.kpi.clicksOfVisitors", { clicks: ctaClicksInRange, visits: visitsInRange })
          : t("analytics.kpi.noVisitData"),
    },
  ];
  ...
  return {
    ...
    stageDistribution,
    outcomes: [
      { label: t("analytics.outcomes.won"), value: wonCount, kind: "won" },
      { label: t("analytics.outcomes.inProgress"), value: openCount, kind: "open" },
      { label: t("analytics.outcomes.lost"), value: lostCount, kind: "lost" },
    ],
    responseBreakdown,
    formCompleteness,
    formStats: {
