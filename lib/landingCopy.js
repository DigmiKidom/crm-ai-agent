...
const FALLBACKS = {
  en: {
    ...
    catalogPhotoAlt: "{title}",
    // The item list's only call to action. Deliberately "ask about this"
    // rather than "buy": nothing on this page can take a payment, and a
    // button that implies otherwise would be a promise the product can't
    // keep. It scrolls to the lead form, which is what the business wants.
    catalogInquire: "Ask about this",
  ...
  he: {
    ...
    catalogHeading: "מה אנחנו מציעים",
    ...
    catalogPhotoAlt: "{title}",
    catalogInquire: "שאלו אותנו על זה",
...
export function resolveLandingCopy(tenant) {
  ...
  return {
    ...
    catalogHeading: pick(landingPage.catalogHeading, pack.catalogHeading),
    // No fallback: the note is a caveat only some businesses have ("prices
    // include VAT"), and inventing one for the rest would put words in their
    // mouth.
    catalogNote: EMPTY(landingPage.catalogNote) ? "" : landingPage.catalogNote,
    // Only rows with a name reach a visitor. A card showing a price and a
    // photo but nothing to call it is not something anyone can ask about —
    // and normalizeCatalog() already rejects it on save, so this only ever
    // catches data written before that existed.
    catalogItems: (landingPage.catalog || [])
    ...
    catalogInquireLabel: pack.catalogInquire,
    catalogPhotoAlt: (title) => pack.catalogPhotoAlt.replace("{title}", title),
