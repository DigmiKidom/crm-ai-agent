...
export default async function AnalyticsPage({ params, searchParams }) {
  ...
  // Locale-prefixed here rather than inside RangePicker: that component
  // renders on the server and so has no LocaleProvider to read from.
  const basePath = localePath(locale, `/t/${tenantSlug}/analytics`);
  const settingsPath = localePath(locale, `/t/${tenantSlug}/settings`);
    ...
    <div>
      ...
      {rangeWasLocked && (
        <p className={styles.upgradeBanner} role="status">
          ...
          <Link href={settingsPath}>{t("settings.billing.upgrade")}</Link>
        </p>
      )}
